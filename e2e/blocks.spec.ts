...
test('block code toggle swaps the preview for the matching renderer source', async ({ page }) => {
  ...
  await expect(panel.locator('[data-code-file="/src/ui/sidebar.ts"]')).toHaveCount(1)
  await expect(panel.locator('[data-code-file="/src/demo/blocks/dashboard-echarts.ts"]')).toHaveCount(1)
  ...
  await expect(panel.locator('[data-code-view] pre code')).toContainText('const dashboard', { timeout: 30_000 })
