...
export const orderDetail = <Message>(h: HtmlBuilder<Message>): Html =>
  masterDetailPage({
    ...
    ], class: 'gap-3' }, h)], description: 'Most recent first', heading: 'Activity', surface: 'plain' }, h),
    detailBehavior: 'stack',
    ...
    master: [
      ...
      ], class: 'gap-3' }, h)], heading: 'Delivery', surface: 'card' }, h),
    ],
    theme: 'comfortable',
  }, h)
...
export const checkoutForm = <Message>(props: Readonly<{
  ...
  return masterDetailPage({
    detail: section({ children: [stack({ children: [
      ...
      ].map(([name, qty, price]) => item({ children: [itemMedia({ children: [icon({ name: 'image' }, h)], variant: 'icon' }, h), itemContent({ children: [itemTitle({ children: [name ?? ''] }, h), itemDescription({ children: [`Qty ${qty ?? ''}`] }, h)] }, h), text({ children: [price ?? ''], numeric: 'tabular', variant: 'label' }, h)], size: 'sm' }, h)), class: 'gap-3' }, h),
      separator({}, h),
