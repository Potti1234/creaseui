...
import type { ChartMessage } from '@/ui/integrations/echarts'
import {
  box,
  dashboardShell,
  grid,
...
export const executiveSummaryDashboard = <Message>(toMessage: (message: ChartMessage) => Message, h: HtmlBuilder<Message>): Html =>
  masterDetailPage({
    ...
    ], class: 'gap-3' }, h)], description: 'Generated from weekly operating data', heading: 'What changed & why', surface: 'plain' }, h),
...
export const projectStatusDashboard = <Message>(toMessage: (message: ChartMessage) => Message, h: HtmlBuilder<Message>): Html =>
  dashboardShell({
    content: [
      ...
      section({ children: [itemGroup({ children: [narrative('Partner API approval', 'External review is two days behind the launch plan.', 'circle-alert', h), narrative('Migration throughput', 'Large accounts need a second batch window.', 'timer', h)], class: 'gap-3' }, h)], heading: 'Blockers & risks', surface: 'card' }, h),
    ],
...
export const serviceMonitoringDashboard = <Message>(toMessage: (message: ChartMessage) => Message, h: HtmlBuilder<Message>): Html =>
  masterDetailPage({
    detail: stack({ children: [
      section({ children: [itemGroup({ children: [narrative('Elevated latency', 'api-eu-west · started 8m ago', 'circle-alert', h), narrative('Queue saturation', 'worker-billing · started 21m ago', 'circle-alert', h)], class: 'gap-3' }, h)], heading: 'Active alerts' }, h),
      ...
      ].map(([name, uptime, status]) => item({ children: [itemContent({ children: [itemTitle({ children: [String(name)] }, h), itemDescription({ children: [String(uptime)] }, h)] }, h), badge({ children: [String(status)], variant: status === 'Healthy' ? 'secondary' : 'outline' }, h)], size: 'sm' }, h)), class: 'gap-3' }, h)], heading: 'Service health' }, h),
    ], gap: 'lg' }, h),
    detailBehavior: 'stack',
...
export const incidentConsoleDashboard = <Message>(h: HtmlBuilder<Message>): Html =>
  masterDetailPage({
    detail: section({ children: [stack({ children: [
      ...
      itemGroup({ children: [narrative('14:47', 'Traffic shifted away from eu-west-1.', 'repeat', h), narrative('14:39', 'Database connection pool reached capacity.', 'file', h), narrative('14:32', 'Latency alert triggered for checkout API.', 'circle-alert', h)], class: 'gap-3' }, h),
