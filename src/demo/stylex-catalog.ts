...
export const catalogSpecimens = <Msg>(
  ...
  return [
    ...
    { name: 'alert', note: 'Destructive variant', view: Alert.alert({ announcement: 'static', variant: 'destructive', children: [Alert.alertTitle({ children: ['Heads up'] }, h), Alert.alertDescription({ children: ['Agent constraints are enforced at lint time.'] }, h)] }, h) },
    ...
    { name: 'field', note: 'Linked control composition', view: Field.controlField({ id: 'stylex-field', label: 'Repository', description: 'A semantic field composition.', toControl: (parts, controlH) => Input.input({ id: parts.controlId, value: 'creaseui', ...(parts.describedBy === undefined ? {} : { describedBy: parts.describedBy }), isInvalid: parts.isInvalid, isDisabled: parts.isDisabled }, controlH) }, h) },
    { name: 'form', note: 'Native form with linked field', view: Form.form({ ariaLabel: 'Catalog form', children: [Field.controlField({ id: 'stylex-form-input', label: 'Project name', description: 'Used for generated artifacts.', toControl: (parts, controlH) => Input.input({ id: parts.controlId, name: 'project', autocomplete: 'off', value: 'creaseui', ...(parts.describedBy === undefined ? {} : { describedBy: parts.describedBy }) }, controlH) }, h)] }, h) },
    ...
    { name: 'menubar', note: 'Static menu triggers', view: Menubar.menubar({ ariaLabel: 'Application menu', menus: [{ id: 'file', label: 'File', model: DropdownMenu.init({ id: 'stylex-catalog-file-menu', isAnimated: true }), toParentMessage: noop, items: ['New', 'Save'], itemToConfig: item => ({ label: item }) }] }, h) },
