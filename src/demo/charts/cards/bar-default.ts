...
export const barTooltip = (
  ...
  formatter: (params: TooltipComponentFormatterCallbackParams) => {
    ...
    const body = rows
      .map(row => {
        const color =
          typeof row.color === 'string' ? row.color : theme.chart1
        const indicator =
...
export const standardBarCard = <Msg>(props: Readonly<{
  hostId: string
  title: string
  ariaLabel: string
...
export const view = <Msg>(
  ...
  standardBarCard({
    hostId: HOST_ID,
    title: 'Bar Chart',
    ariaLabel: 'Bar chart showing desktop visitors from January through June 2024',
    toMessage,
  })
