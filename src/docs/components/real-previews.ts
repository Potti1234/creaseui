...
const previews: Readonly<Record<string, (model: State.Model) => Html>> = {
  ...
  checkbox: model => Checkbox.checkbox({ id: 'docs-checkbox', isChecked: model.isCheckboxChecked, onToggle: isChecked => State.ToggledCheckbox({ isChecked }), label: 'Accept terms and conditions', description: 'You agree to our Terms of Service and Privacy Policy.' }),
  collapsible: model => Collapsible.collapsible({ id: 'docs-collapsible', isOpen: model.isCollapsibleOpen, onToggle: isOpen => State.ToggledCollapsible({ isOpen }), class: 'w-full max-w-sm space-y-2', triggerClass: 'flex w-full items-center justify-between rounded-md border px-4 py-2 text-sm font-semibold', trigger: '@peduarte starred 3 repositories', contentClass: 'space-y-2', content: html<PreviewMessage>().div([html<PreviewMessage>().Class('rounded-md border px-4 py-3 text-sm')], ['@radix-ui/primitives']) }),
  ...
  'radio-group': model => RadioGroup.radioGroup({ id: 'docs-radio', selectedValue: Option.some(model.selectedRadioValue), onSelect: value => State.SelectedRadioValue({ value }), ariaLabel: 'Density', options: [{ value: 'default', label: 'Default' }, { value: 'comfortable', label: 'Comfortable' }, { value: 'compact', label: 'Compact' }] }),
  ...
  slider: model => Slider.slider({ model: model.slider, value: model.sliderValue, toParentMessage: message => State.GotSliderMessage({ message }), ariaLabel: 'Volume', class: 'w-full max-w-sm' }),
  spinner: () => Spinner.spinner({ class: 'size-6' }),
  switch: model => Switch.switchControl({ id: 'docs-switch', isChecked: model.isSwitchChecked, onToggle: isChecked => State.ToggledSwitch({ isChecked }), label: 'Airplane Mode' }),
  table: invoiceTable,
  tabs: model => Tabs.tabs({ model: model.tabs, selectedValue: model.selectedTabValue, toParentMessage: message => State.GotTabsMessage({ message }), class: 'w-full max-w-md', tabs: [{ value: 'account', label: 'Account', content: Card.card({ children: [Card.cardHeader({ children: [Card.cardTitle({ children: ['Account'] }), Card.cardDescription({ children: ['Make changes to your account here.'] })] })] }) }, { value: 'password', label: 'Password', content: Card.card({ children: [Card.cardHeader({ children: [Card.cardTitle({ children: ['Password'] }), Card.cardDescription({ children: ['Change your password here.'] })] })] }) }] }),
  ...
  combobox: model => Combobox.combobox({ model: model.combobox, maybeSelectedValue: model.selectedComboboxValue, restingInputValue: Option.match(model.selectedComboboxValue, { onNone: () => '', onSome: value => ({ next: 'Next.js', svelte: 'SvelteKit', nuxt: 'Nuxt.js' })[value as 'next' | 'svelte' | 'nuxt'] ?? value }), toParentMessage: message => State.GotComboboxMessage({ message }), items: [{ value: 'next', label: 'Next.js' }, { value: 'svelte', label: 'SvelteKit' }, { value: 'nuxt', label: 'Nuxt.js' }], itemToValue: item => item.value, itemToLabel: item => item.label, placeholder: 'Select framework…' }),
  command: model => CommandMenu.command({ model: model.command, maybeSelectedValue: model.selectedCommandValue, restingInputValue: Option.match(model.selectedCommandValue, { onNone: () => '', onSome: value => value[0]?.toUpperCase() + value.slice(1) }), toParentMessage: message => State.GotCommandMessage({ message }), class: 'w-full max-w-md border shadow-md', items: ['calendar', 'search', 'settings'], itemToConfig: item => ({ content: item[0]?.toUpperCase() + item.slice(1) }), placeholder: 'Type a command or search…' }),
  ...
  'date-picker': model => DatePicker.datePicker({ model: model.datePicker, maybeSelectedDate: model.selectedDate, toParentMessage: message => State.GotDatePickerMessage({ message }) }),
  ...
  select: model => Select.select({ model: model.select, maybeSelectedValue: model.selectedSelectValue, toParentMessage: message => State.GotSelectMessage({ message }), items: [{ value: 'apple', label: 'Apple' }, { value: 'banana', label: 'Banana' }, { value: 'blueberry', label: 'Blueberry' }], itemToValue: item => item.value, itemToLabel: item => item.label, placeholder: 'Select a fruit' }),
