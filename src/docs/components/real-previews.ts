...
export const PreviewInteracted = m('PreviewInteracted')
export const Message = S.Union([PreviewInteracted])
export type Message = typeof Message.Type
type PreviewMessage = Message
const interacted: PreviewMessage = PreviewInteracted()
...
export const realPreview = (slug: string): Html => previews[slug]?.() ?? html<PreviewMessage>().div([], [])

const previews: Readonly<Record<string, () => Html>> = {
  ...
  card: () => Card.card({ class: 'w-full max-w-sm', children: [
    ...
    Card.cardContent({ children: [Input.input({ id: 'card-email', value: '', onInput: () => interacted, label: 'Email', placeholder: 'm@example.com' })] }),
    Card.cardFooter({ children: [Button.button({ class: 'w-full', children: ['Login'], onClick: interacted })] }),
  ] }),
  carousel: () => Carousel.carousel({ model: Carousel.init('docs-carousel', 3), toParentMessage: () => interacted, class: 'w-full max-w-xs', items: [1, 2, 3].map(number => Card.card({ children: [Card.cardContent({ class: 'flex aspect-square items-center justify-center p-6 text-4xl font-semibold', children: [String(number)] })] })) }),
  ...
  checkbox: () => Checkbox.checkbox({ model: Checkbox.init({ id: 'docs-checkbox', isChecked: true }), toParentMessage: () => interacted, label: 'Accept terms and conditions', description: 'You agree to our Terms of Service and Privacy Policy.' }),
  collapsible: () => Collapsible.collapsible({ model: Collapsible.init({ id: 'docs-collapsible', isOpen: true }), toParentMessage: () => interacted, class: 'w-full max-w-sm space-y-2', triggerClass: 'flex w-full items-center justify-between rounded-md border px-4 py-2 text-sm font-semibold', trigger: '@peduarte starred 3 repositories', contentClass: 'space-y-2', content: html<PreviewMessage>().div([html<PreviewMessage>().Class('rounded-md border px-4 py-3 text-sm')], ['@radix-ui/primitives']) }),
  ...
  field: () => Field.fieldGroup({ class: 'w-full max-w-sm', children: [Field.field({ children: [Field.fieldLabel({ for: 'docs-field-name', children: ['Name'] }), Input.input({ id: 'docs-field-name', value: '', onInput: () => interacted, placeholder: 'Evil Rabbit' }), Field.fieldDescription({ children: ['This is your public display name.'] })] })] }),
  form: () => Form.form({ class: 'w-full max-w-sm', onSubmit: interacted, children: [Form.formItem({ id: 'docs-form-email', children: [Form.formLabel({ for: 'docs-form-email', children: ['Email'] }), Input.input({ id: 'docs-form-email', value: '', onInput: () => interacted, placeholder: 'm@example.com' }), Form.formDescription({ children: ['We will never share your email.'] })] }), Button.button({ type: 'submit', children: ['Submit'] })] }),
  input: () => Input.input({ id: 'docs-input', value: '', onInput: () => interacted, placeholder: 'Email', class: 'max-w-sm' }),
  'input-group': () => InputGroup.inputGroup({ class: 'max-w-sm', children: [InputGroup.inputGroupAddon({ children: [InputGroup.inputGroupText({ children: ['https://'] })] }), InputGroup.inputGroupInput({ id: 'docs-input-group', value: '', onInput: () => interacted, placeholder: 'example.com' })] }),
  'input-otp': () => InputOtp.inputOtp({ id: 'docs-otp', value: '123', onInput: () => interacted, separator: index => index === 2 ? InputOtp.inputOtpSeparator() : html<PreviewMessage>().span([], []) }),
  ...
  label: () => html<PreviewMessage>().div([html<PreviewMessage>().Class('grid w-full max-w-sm gap-2')], [Label.label({ for: 'docs-label-input', children: ['Email'] }), Input.input({ id: 'docs-label-input', value: '', onInput: () => interacted, placeholder: 'Email' })]),
  ...
  'native-select': () => NativeSelect.nativeSelect({ id: 'docs-native-select', value: 'apple', onChange: () => interacted, options: [{ value: 'apple', label: 'Apple' }, { value: 'banana', label: 'Banana' }, { value: 'blueberry', label: 'Blueberry' }], class: 'max-w-xs' }),
  ...
  'radio-group': () => RadioGroup.radioGroup({ model: RadioGroup.init({ id: 'docs-radio', selectedValue: 'comfortable' }), toParentMessage: () => interacted, ariaLabel: 'Density', options: [{ value: 'default', label: 'Default' }, { value: 'comfortable', label: 'Comfortable' }, { value: 'compact', label: 'Compact' }] }),
  resizable: () => Resizable.resizable({ model: Resizable.init('docs-resizable', 50), toParentMessage: () => interacted, class: 'h-48 w-full max-w-xl', withHandle: true, first: html<PreviewMessage>().div([html<PreviewMessage>().Class('flex size-full items-center justify-center p-6')], ['One']), second: html<PreviewMessage>().div([html<PreviewMessage>().Class('flex size-full items-center justify-center p-6')], ['Two']) }),
  ...
  slider: () => Slider.slider({ model: Slider.init({ id: 'docs-slider', min: 0, max: 100, step: 1, initialValue: 50 }), toParentMessage: () => interacted, ariaLabel: 'Volume', class: 'w-full max-w-sm' }),
  spinner: () => Spinner.spinner({ class: 'size-6' }),
  switch: () => Switch.switchControl({ model: Switch.init({ id: 'docs-switch', isChecked: true }), toParentMessage: () => interacted, label: 'Airplane Mode' }),
  table: invoiceTable,
  tabs: () => Tabs.tabs({ model: Tabs.init({ id: 'docs-tabs', activeIndex: 0 }), toParentMessage: () => interacted, class: 'w-full max-w-md', tabs: [{ value: 'account', label: 'Account', content: Card.card({ children: [Card.cardHeader({ children: [Card.cardTitle({ children: ['Account'] }), Card.cardDescription({ children: ['Make changes to your account here.'] })] })] }) }, { value: 'password', label: 'Password', content: Card.card({ children: [Card.cardHeader({ children: [Card.cardTitle({ children: ['Password'] }), Card.cardDescription({ children: ['Change your password here.'] })] })] }) }] }),
  textarea: () => Textarea.textarea({ id: 'docs-textarea', value: '', onInput: () => interacted, placeholder: 'Type your message here.', class: 'max-w-sm' }),
  toggle: () => Toggle.toggle({ isPressed: true, onToggle: interacted, children: ['Bold'] }),
  'toggle-group': () => ToggleGroup.toggleGroup({ value: 'center', onToggle: () => interacted, variant: 'outline', items: [{ value: 'left', children: ['Left'] }, { value: 'center', children: ['Center'] }, { value: 'right', children: ['Right'] }] }),
  ...
  'alert-dialog': () => AlertDialog.alertDialog({ model: AlertDialog.open(AlertDialog.init({ id: 'docs-alert-dialog', isAnimated: false }))[0], toParentMessage: () => interacted, title: 'Are you absolutely sure?', description: 'This action cannot be undone. This will permanently delete your account.', actionLabel: 'Continue' }),
  combobox: () => Combobox.combobox({ model: Combobox.init({ id: 'docs-combobox', isAnimated: false }), toParentMessage: () => interacted, items: [{ value: 'next', label: 'Next.js' }, { value: 'svelte', label: 'SvelteKit' }, { value: 'nuxt', label: 'Nuxt.js' }], itemToValue: item => item.value, itemToLabel: item => item.label, placeholder: 'Select framework…' }),
  command: () => CommandMenu.command({ model: CommandMenu.init({ id: 'docs-command', isAnimated: false }), toParentMessage: () => interacted, class: 'w-full max-w-md border shadow-md', items: ['calendar', 'search', 'settings'], itemToConfig: item => ({ content: item[0]?.toUpperCase() + item.slice(1) }), placeholder: 'Type a command or search…' }),
  'context-menu': () => ContextMenu.contextMenu({ model: { ...ContextMenu.init({ id: 'docs-context-menu' }), isOpen: true }, toParentMessage: () => interacted, class: 'flex h-40 w-72 items-center justify-center rounded-md border border-dashed text-sm', trigger: 'Right click here', items: ['back', 'forward', 'reload'], itemToConfig: item => ({ label: item[0]?.toUpperCase() + item.slice(1) }) }),
  'date-picker': () => DatePicker.datePicker({ model: DatePicker.init({ id: 'docs-date-picker', today, initialSelectedDate: { year: 2026, month: 7, day: 18 }, isAnimated: false }), toParentMessage: () => interacted }),
  dialog: () => Dialog.dialog({ model: Dialog.open(Dialog.init({ id: 'docs-dialog', isAnimated: false }))[0], toParentMessage: () => interacted, title: 'Edit profile', description: 'Make changes to your profile here. Click save when you’re done.', content: () => [Input.input({ id: 'docs-dialog-name', value: 'Pedro Duarte', onInput: () => interacted, label: 'Name' })], footer: () => [Button.button({ children: ['Save changes'], onClick: interacted })] }),
  drawer: () => Drawer.drawer({ model: Drawer.open(Drawer.init({ id: 'docs-drawer', isAnimated: false }))[0], toParentMessage: () => interacted, title: 'Move Goal', description: 'Set your daily activity goal.', content: () => [html<PreviewMessage>().div([html<PreviewMessage>().Class('px-4 pb-6 text-center text-5xl font-bold')], ['350'])] }),
  'dropdown-menu': () => DropdownMenu.dropdownMenu({ model: { ...DropdownMenu.init({ id: 'docs-dropdown' }), isOpen: true }, toParentMessage: () => interacted, triggerClass: 'rounded-md border px-4 py-2 text-sm font-medium shadow-xs', trigger: 'Open', items: ['profile', 'billing', 'settings', 'logout'], itemToConfig: item => ({ label: item[0]?.toUpperCase() + item.slice(1), separatorBefore: item === 'logout' }) }),
  'hover-card': () => HoverCard.hoverCard({ model: { ...HoverCard.init({ id: 'docs-hover-card' }), isOpen: true }, toParentMessage: () => interacted, triggerClass: 'underline underline-offset-4', trigger: '@nextjs', content: html<PreviewMessage>().div([html<PreviewMessage>().Class('space-y-1')], [html<PreviewMessage>().h4([html<PreviewMessage>().Class('font-semibold')], ['@nextjs']), html<PreviewMessage>().p([html<PreviewMessage>().Class('text-sm')], ['The React Framework – created and maintained by @vercel.'])]) }),
  'message-scroller': () => MessageScroller.messageScroller({ class: 'h-72 w-full max-w-md rounded-md border', children: [MessageScroller.messageScrollerContent({ children: Array.from({ length: 8 }, (_, index) => MessageScroller.messageScrollerItem({ children: [Bubble.bubble({ align: index % 2 === 0 ? 'start' : 'end', children: [Bubble.bubbleContent({ children: [`Message ${index + 1}`] })] })] })) })] }),
  menubar: () => Menubar.menubar<string, PreviewMessage>({ menus: ['File', 'Edit', 'View'].map((label, index) => ({ id: `docs-menubar-${index}`, label, model: DropdownMenu.init({ id: `docs-menubar-${index}` }), toParentMessage: () => interacted, items: ['new', 'open', 'save'], itemToConfig: item => ({ label: item[0]?.toUpperCase() + item.slice(1) }) })) }),
  popover: () => Popover.popover({ model: Popover.open(Popover.init({ id: 'docs-popover', isAnimated: false }))[0], toParentMessage: () => interacted, triggerClass: 'rounded-md border px-4 py-2 text-sm font-medium', trigger: 'Open popover', content: html<PreviewMessage>().div([html<PreviewMessage>().Class('grid gap-2')], [html<PreviewMessage>().h4([html<PreviewMessage>().Class('font-medium')], ['Dimensions']), html<PreviewMessage>().p([html<PreviewMessage>().Class('text-sm text-muted-foreground')], ['Set the dimensions for the layer.'])]) }),
  select: () => Select.select({ model: Select.init({ id: 'docs-select', selectedItem: 'apple' }), toParentMessage: () => interacted, items: [{ value: 'apple', label: 'Apple' }, { value: 'banana', label: 'Banana' }, { value: 'blueberry', label: 'Blueberry' }], itemToValue: item => item.value, itemToLabel: item => item.label, placeholder: 'Select a fruit' }),
  sheet: () => Sheet.sheet({ model: Sheet.open(Sheet.init({ id: 'docs-sheet', isAnimated: false }))[0], toParentMessage: () => interacted, title: 'Edit profile', description: 'Make changes to your profile here.', content: () => [Input.input({ id: 'docs-sheet-name', value: 'Pedro Duarte', onInput: () => interacted, label: 'Name' })] }),
  tooltip: () => Tooltip.tooltip({ model: Tooltip.init({ id: 'docs-tooltip', showDelay: 0 }), toParentMessage: () => interacted, triggerClass: 'rounded-md border px-3 py-2 text-sm', trigger: 'Hover', content: 'Add to library' }),
  ...
  sonner: () => Sonner.sonner({ model: Sonner.show(Sonner.init({ id: 'docs-sonner' }), Sonner.success({ title: 'Event has been created', description: 'Sunday, December 03, 2023 at 9:00 AM', sticky: true }))[0], toParentMessage: () => interacted, class: 'relative right-auto bottom-auto p-0' }),
  toast: () => Toast.toast({ model: Toast.show(Toast.init({ id: 'docs-toast' }), Toast.info({ title: 'Scheduled: Catch up', description: 'Friday, February 10, 2023 at 5:57 PM', sticky: true }))[0], toParentMessage: () => interacted, class: 'relative right-auto bottom-auto p-0' }),
}
