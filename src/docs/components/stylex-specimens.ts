...
export const catalogSpecimens = <Msg>(
  ...
  return [
    ...
    { name: 'attachment', note: 'Completed upload', view: Attachment.attachment({ state: 'done', children: [Attachment.attachmentMedia({ children: ['PDF'] }, h), Attachment.attachmentContent({ children: [Attachment.attachmentTitle({ children: ['agent-contract.pdf'] }, h), Attachment.attachmentDescription({ children: ['Uploaded Â· 240 KB'] }, h)] }, h)] }, h) },
    ...
    { name: 'direction', note: 'RTL containment', view: Direction.direction({ direction: 'rtl', children: ['ÙˆØ§Ø¬Ù‡Ø© Ù…Ù† Ø§Ù„ÙŠÙ…ÙŠÙ† Ø¥Ù„Ù‰ Ø§Ù„ÙŠØ³Ø§Ø±'] }, h) },
    ...
    { name: 'kbd', note: 'Keyboard hint', view: Kbd.kbdGroup({ children: [Kbd.kbd({ children: ['âŒ˜'] }, h), Kbd.kbd({ children: ['K'] }, h)] }, h) },
    ...
    { name: 'scroll-area', note: 'Contained overflow', view: ScrollArea.scrollArea({ ariaLabel: 'Token list', children: ['background Â· foreground Â· primary Â· accent Â· destructive'] }, h) },
...
export const stylexCatalog = <Msg>(
  ...
  return h.section([h.AriaLabel('StyleX component catalog'), h.Class('mt-16')], [
    h.div([h.Class('flex flex-col gap-3 border-b pb-6 sm:flex-row sm:items-end sm:justify-between')], [
      ...
      h.label([h.Class('grid gap-1 text-xs font-medium')], ['Filter components', h.input([h.Type('search'), h.Value(query), h.OnInput(onQuery), h.Placeholder('Search 65 componentsâ€¦'), h.AriaLabel('Filter StyleX components'), h.Class('h-9 w-full rounded-md border bg-background px-3 text-sm outline-none focus-visible:ring-2 sm:w-64')])]),
    ]),
