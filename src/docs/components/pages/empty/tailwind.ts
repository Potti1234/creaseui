...
export const emptyTailwindPreviewProgram = interactionPreviewProgram('empty', (index, interaction, h, interactionCount) => {
  ...
  return Empty.empty({ class: `w-full max-w-xl${fixture.bordered ? ' border' : ''}`, children: [
    ...
    Empty.emptyContent({ children: [Button.button({ ...(fixture.outline ? { variant: 'outline' as const } : {}), onClick: interaction, children: [fixture.action] }, h), h.p([h.Role('status'), h.Class('text-sm text-muted-foreground')], [`Clicked ${String(interactionCount)} times`])] }, h),
  ] }, h);
});
