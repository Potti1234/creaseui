...
export const chartPage = authoredPage({
  ...
  definition: {
    ...
    examples: [
      { title: 'Monthly revenue', description: 'A responsive bar chart is a pure projection of typed data and needs no update branch.', preview: (_model, h) => Chart.barChart({ class: 'max-w-xl', data: [{ label: 'Jan', value: 186 }, { label: 'Feb', value: 305 }, { label: 'Mar', value: 237 }, { label: 'Apr', value: 273 }, { label: 'May', value: 209 }, { label: 'Jun', value: 314 }] }, h), code: barSource, previewClass: 'justify-stretch' },
      { title: 'Traffic trend', description: 'Area rendering and the shared legend remain separate helpers so the application controls composition.', preview: (_model, h) => h.div([h.Class('w-full max-w-xl space-y-4')], [Chart.areaChart({ data: [186, 305, 237, 273, 209, 314] }, h), Chart.chartLegend({ config: { visitors: { label: 'Visitors', color: 'var(--chart-2)' } } }, h)]), code: areaSource, previewClass: 'justify-stretch' },
    ],
  },
});
