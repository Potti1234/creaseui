...
export const dataTableTailwindPreviewProgram = definePreviewProgram<Model, Got>({
  ...
  view: (index, model, h) => { const fixture = dataTableFixtures[index] ?? dataTableFixtures[0]; return DataTable.dataTable({ id: `docs-data-table-${String(index)}`, model: model.table, toParentMessage: message => Got['GotDataTablePreviewMessage']({ message }), rows: payments, columns: columns(), rowKey: row => row.id, ...(fixture.filter ? { filterText: (row: Payment) => `${row.status} ${row.email}`, filterPlaceholder: 'Filter payments…' } : {}), ...(fixture.server ? { mode: 'server' as const, rowCount: 42 } : {}), enableRowSelection: true, enableColumnVisibility: true, pageSizeOptions: [5, 10, 20], ariaLabel: 'Payments' }, h); },
});
