...
export const init = (): Model => ({
  ...
  customSize: Calendar.init({
    id: 'docs-calendar-custom-size',
    today: date(2026, 7, 28),
  }),
  rtl: Calendar.init({
    id: 'docs-calendar-rtl',
    today: date(2026, 7, 28),
    initialViewDate: date(2026, 7, 18),
  }),
  selectedDates: {
  ...
  return [
    {
      ...model,
      [target]: calendar,
      selectedDates: { ...model.selectedDates, [target]: selectedDate },
    },
  ...
  return componentPage<Message>(
    {
      ...
      examples: [
        docsExample({
          ...
          preview: preview(
            model.basic,
            'basic',
            model.selectedDates.basic ?? Option.none(),
            h,
          ),
                ...
                Card.cardContent(
                  {
                    ...
                    children: [
                      preview(
                        model.selected,
                        'selected',
                        model.selectedDates.selected ?? Option.none(),
                        h,
                      ),
                    ],
                  },
        ...
        docsExample({
          ...
          description:
            'Weekend dates remain visible while unavailable for selection.',
          preview: h.div(
            ...
            [
              preview(
                model.disabled,
                'disabled',
                model.selectedDates.disabled ?? Option.none(),
                h,
              ),
            ],
          ),
        ...
        docsExample({
          ...
          description:
            'Override the calendar cell token to create a roomier picker without changing its behavior.',
          preview: Calendar.calendar(
            {
              model: model.customSize,
              maybeSelectedDate:
                model.selectedDates.customSize ?? Option.none(),
              toParentMessage: (message) =>
                GotCalendarMessage({ target: 'customSize', message }),
        ...
        docsExample({
          title: 'RTL',
          description:
            'Calendar navigation, headings, and grid interaction follow the surrounding text direction.',
          preview: Direction.direction(
            {
              ...
              children: [
                preview(
                  model.rtl,
                  'rtl',
                  model.selectedDates.rtl ?? Option.none(),
                  h,
                ),
              ],
            },
      ...
      sourceHref:
        'https://github.com/Potti1234/creaseui/blob/main/src/ui/calendar.ts',
      apiDescription:
        'Calendar delegates focus, keyboard navigation, date selection, and view-mode changes to the Foldkit Calendar state machine.',
    },
    h,
  );
};
