...
export const Message = defineMessageUnion({
  Scrolled: { scrollTop: S.Number, scrollHeight: S.Number, clientHeight: S.Number },
  'ObservedMessageScrollerViewport': {
  reason: S.Literals(['mount', 'user', 'content', 'resize']),
  scrollTop: S.Number,
  scrollHeight: S.Number,
  clientHeight: S.Number,
},
  RequestedScroll: { direction: S.Literals(['start', 'end']) },
  CompletedMessageScrollerScrollTo: { version: S.Number },
});
...
const ScrollTo = Command.define('MessageScrollerScrollTo', {
  ...
  messages: [Message.CompletedMessageScrollerScrollTo],
  ...
  }).pipe(Effect.as(Message.CompletedMessageScrollerScrollTo({ version }))),
})
...
const requestScroll = (model: Model, direction: 'start' | 'end', immediate: boolean): UpdateReturn => {
  ...
  return { model: { ...model, isFollowing: direction === 'end', hasNewMessages: direction === 'end' ? false : model.hasNewMessages, pendingScrollVersion: Option.some(version) }, commands: [ScrollTo({ id: model.id, direction, version, immediate })] }
}

export const update = (model: Model, message: Message): UpdateReturn => {
  switch (message._tag) {
    case 'Scrolled': return update(model, Message['ObservedMessageScrollerViewport']({ scrollTop: message.scrollTop, scrollHeight: message.scrollHeight, clientHeight: message.clientHeight, reason: 'user' }))
    case 'ObservedMessageScrollerViewport': {
      ...
      if (message.reason === 'user') return { model: { ...model, ...metrics, isReady: true, isFollowing: isAtEnd, hasNewMessages: isAtEnd ? false : model.hasNewMessages } }
      if (!model.isFollowing) return { model: { ...model, ...metrics, isReady: true, hasNewMessages: message.reason === 'content' ? true : model.hasNewMessages } }
    ...
    case 'CompletedMessageScrollerScrollTo': return model.pendingScrollVersion._tag === 'Some' && model.pendingScrollVersion.value === message.version
      ? { model: { ...model, pendingScrollVersion: Option.none(), isReady: true }, }
      : ({ model: model })
  }
}

const measure = (element: HTMLElement, reason: 'mount' | 'user' | 'content' | 'resize') => Message['ObservedMessageScrollerViewport']({ reason, scrollTop: element.scrollTop, scrollHeight: element.scrollHeight, clientHeight: element.clientHeight })

export const ObserveMessageScroller = Mount.defineStream('ObserveMessageScroller', {
  messages: [Message.ObservedMessageScrollerViewport],
  execute: ({ element }) =>
    Stream.callback<typeof Message.ObservedMessageScrollerViewport.Type>(queue => Effect.gen(function* () {
