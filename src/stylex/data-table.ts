...
export type DataTableProps<Row, Msg> = Readonly<{ model: Model; toParentMessage: (message: Message) => Msg; rows: ReadonlyArray<Row>; columns: ReadonlyArray<DataTableColumn<Row>>; rowKey: (row: Row) => string; filterText?: (row: Row) => string; filterPlaceholder?: string; emptyText?: string; ariaLabel?: string; enableRowSelection?: boolean; enableColumnVisibility?: boolean; isRowSelectable?: (row: Row) => boolean; rowSelectionLabel?: (row: Row) => string; pageSizeOptions?: ReadonlyArray<number>; layoutStyle?: ComponentLayoutStyle }>
...
const compare = (left: string | number, right: string | number): number => typeof left === 'number' && typeof right === 'number' ? left - right : String(left).localeCompare(String(right), undefined, { numeric: true, sensitivity: 'base' })
...
export const dataTable = <Row, Msg>(props: DataTableProps<Row, Msg>, h: HtmlBuilder<Msg>): Html => {
  ...
  const filtered = query === '' || props.filterText === undefined ? [...props.rows] : props.rows.filter((row) => props.filterText?.(row).toLocaleLowerCase().includes(query) === true)
  const sortColumn = props.columns.find((column) => column.key === props.model.sortKey)
  const sorted = sortColumn?.sortValue === undefined ? filtered : [...filtered].sort((left, right) => compare(sortColumn.sortValue?.(left) ?? '', sortColumn.sortValue?.(right) ?? '') * (props.model.sortDirection === 'ascending' ? 1 : -1))
  ...
  const selectableRows = visible.filter((row) => props.isRowSelectable?.(row) ?? true)
  ...
  const selectedCount = props.rows.map(props.rowKey).filter((key) => selectedKeys.has(key)).length
  ...
  return h.div([h.DataAttribute('slot', 'data-table'), ...(props.ariaLabel === undefined ? [] : [h.AriaLabel(props.ariaLabel)]), h.Class(className(styles.root, props.layoutStyle))], [
    h.div([h.Class(className(styles.toolbar))], [
      ...(props.filterText === undefined ? [] : [h.input([h.Type('search'), h.Value(props.model.filter), h.OnInput((value) => props.toParentMessage(Filtered({ value }))), h.Placeholder(props.filterPlaceholder ?? 'Filter rowsâ€¦'), h.AriaLabel(props.filterPlaceholder ?? 'Filter rows'), h.Class(className(styles.filter))])]),
    ...
    h.div([h.Class(className(styles.tableShell))], [h.table([h.DataAttribute('slot', 'table'), h.Class(className(styles.table))], [
      h.thead([h.Class(className(styles.header))], [h.tr([], [...(props.enableRowSelection === true ? [h.th([h.Scope('col'), h.Class(className(styles.selectionCell))], [selectionControl({ id: 'data-table-select-page', label: allOnPageSelected ? 'Deselect all rows on this page' : 'Select all rows on this page', isChecked: allOnPageSelected, isIndeterminate: someOnPageSelected, isDisabled: selectableKeys.length === 0, onToggle: (isSelected) => props.toParentMessage(ToggledRows({ keys: selectableKeys, isSelected })) }, h)])] : []), ...headings])]),
      h.tbody([], bodyRows),
    ])]),
    h.div([h.Class(className(styles.footer))], [h.p([h.Class(className(styles.count))], [props.enableRowSelection === true ? `${selectedCount} of ${filtered.length} row${filtered.length === 1 ? '' : 's'} selected.` : `${filtered.length} row${filtered.length === 1 ? '' : 's'}`]), h.div([h.Class(className(styles.controls))], [h.label([h.Class(className(styles.count)), h.For('data-table-page-size')], ['Rows per page']), h.select([h.Id('data-table-page-size'), h.Value(String(props.model.pageSize)), h.OnChange((value) => props.toParentMessage(ChangedPageSize({ pageSize: Number(value) }))), h.Class(className(styles.select))], pageSizes.map((size) => h.option([h.Value(String(size))], [String(size)]))), h.span([h.Class(className(styles.pageLabel))], [`Page ${page + 1} of ${pageCount}`]), h.button([h.Type('button'), h.AriaLabel('First page'), h.Disabled(page === 0), h.OnClick(props.toParentMessage(ChangedPage({ page: 0 }))), h.Class(className(styles.button))], ['First']), h.button([h.Type('button'), h.AriaLabel('Previous page'), h.Disabled(page === 0), h.OnClick(props.toParentMessage(ChangedPage({ page: page - 1 }))), h.Class(className(styles.button))], ['Previous']), h.button([h.Type('button'), h.AriaLabel('Next page'), h.Disabled(page >= pageCount - 1), h.OnClick(props.toParentMessage(ChangedPage({ page: page + 1 }))), h.Class(className(styles.button))], ['Next']), h.button([h.Type('button'), h.AriaLabel('Last page'), h.Disabled(page >= pageCount - 1), h.OnClick(props.toParentMessage(ChangedPage({ page: pageCount - 1 }))), h.Class(className(styles.button))], ['Last'])])]),
  ])
}
