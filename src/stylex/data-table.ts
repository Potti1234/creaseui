...
export const dataTable = <Row, Msg>(props: DataTableProps<Row, Msg>, h: HtmlBuilder<Msg>): Html => {
  ...
  return h.div([h.DataAttribute('slot', 'data-table'), ...(props.ariaLabel === undefined ? [] : [h.AriaLabel(props.ariaLabel)]), h.Class(className(styles.root, props.layoutStyle))], [
    h.div([h.Class(className(styles.toolbar))], [
      ...(props.filterText === undefined ? [] : [h.input([h.Type('search'), h.Value(props.model.filter), h.OnInput((value) => props.toParentMessage(DataTableStateMessages.Filtered({ value }))), h.Placeholder(props.filterPlaceholder ?? 'Filter rows…'), h.AriaLabel(props.filterPlaceholder ?? 'Filter rows'), h.Class(className(styles.filter))])]),
      ...(props.enableColumnVisibility === true ? [h.div([h.Class(className(styles.chooser)), h.OnKeyDownPreventDefault((key) => key === 'Escape' && props.model.columnsMenuOpen ? Option.some(props.toParentMessage(DataTableStateMessages.ClosedColumnsMenu())) : Option.none())], [
        h.button([h.Type('button'), h.AriaExpanded(props.model.columnsMenuOpen), h.AriaHasPopup('menu'), h.OnClick(props.toParentMessage(DataTableStateMessages.ToggledColumnsMenu())), h.Class(className(styles.chooserSummary))], ['Columns', Icon.chevronDown<Msg>({ class: className(styles.sortIcon) }, h)]),
        ...(props.model.columnsMenuOpen ? [
          h.button([h.Type('button'), h.Tabindex(-1), h.AriaHidden(true), h.Style({ position: 'fixed', inset: '0', zIndex: '10', cursor: 'default', border: 'none', background: 'transparent' }), h.OnClick(props.toParentMessage(DataTableStateMessages.ClosedColumnsMenu()))], []),
          h.div([h.Class(className(styles.chooserPanel))], props.columns.filter((column) => column.isHideable !== false && column.header.trim() !== '').map((column) => { const isVisible = !hiddenKeys.has(column.key); return h.div([h.Class(className(styles.chooserOption))], [selectionControl({ id: `data-table-column-${column.key}`, label: `${isVisible ? 'Hide' : 'Show'} ${column.header} column`, isChecked: isVisible, isDisabled: isVisible && visibleColumns.length === 1, onToggle: (nextVisible) => props.toParentMessage(DataTableStateMessages.ToggledColumn({ key: column.key, isVisible: nextVisible })) }, h), column.header]) })),
        ] : []),
      ])] : []),
    ]),
    h.div([h.Class(className(styles.tableShell))], [Table.table({ children: [
      h.thead([h.Class(className(styles.header))], [h.tr([], [...(props.enableRowSelection === true ? [h.th([h.Scope('col'), h.Class(className(styles.selectionCell))], [selectionControl({ id: `${instanceId}-select-page`, label: allOnPageSelected ? 'Deselect all rows on this page' : 'Select all rows on this page', isChecked: allOnPageSelected, isIndeterminate: someOnPageSelected, isDisabled: selectableKeys.length === 0, onToggle: (isSelected) => props.toParentMessage(DataTableStateMessages.ToggledRows({ keys: selectableKeys, isSelected })) }, h)])] : []), ...headings])]),
      h.tbody([], bodyRows),
    ] }, h)]),
    h.div([h.Class(className(styles.footer))], [h.p([h.Class(className(styles.count))], [props.enableRowSelection === true ? `${selectedCount} of ${filteredRowCount} row${filteredRowCount === 1 ? '' : 's'} selected.` : `${filteredRowCount} row${filteredRowCount === 1 ? '' : 's'}`]), h.div([h.Class(className(styles.controls))], [h.label([h.Class(className(styles.count)), h.For(`${instanceId}-page-size`)], ['Rows per page']), h.select([h.Id(`${instanceId}-page-size`), h.Value(String(props.model.pageSize)), h.OnChange((value) => props.toParentMessage(DataTableStateMessages.ChangedPageSize({ pageSize: Number(value) }))), h.Class(className(styles.select))], pageSizes.map((size) => h.option([h.Value(String(size))], [String(size)]))), h.span([h.Class(className(styles.pageLabel))], [`Page ${page + 1} of ${pageCount}`]), h.button([h.Type('button'), h.AriaLabel('First page'), h.Disabled(page === 0), h.OnClick(props.toParentMessage(DataTableStateMessages.ChangedPage({ page: 0 }))), h.Class(className(styles.button))], ['First']), h.button([h.Type('button'), h.AriaLabel('Previous page'), h.Disabled(page === 0), h.OnClick(props.toParentMessage(DataTableStateMessages.ChangedPage({ page: page - 1 }))), h.Class(className(styles.button))], ['Previous']), h.button([h.Type('button'), h.AriaLabel('Next page'), h.Disabled(page >= pageCount - 1), h.OnClick(props.toParentMessage(DataTableStateMessages.ChangedPage({ page: page + 1 }))), h.Class(className(styles.button))], ['Next']), h.button([h.Type('button'), h.AriaLabel('Last page'), h.Disabled(page >= pageCount - 1), h.OnClick(props.toParentMessage(DataTableStateMessages.ChangedPage({ page: pageCount - 1 }))), h.Class(className(styles.button))], ['Last'])])]),
  ])
}
