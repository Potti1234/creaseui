...
const calendarMonth = <Message>(year: number, month: number, model: State.Model, toParentMessage: (message: State.Message) => Message, navigation: 'previous' | 'next', h: HtmlBuilder<Message>): Html => {
  ...
  return h.div([h.Class(className(styles.calendarMonth))], [
    h.div([h.Class(className(styles.calendarTitleRow))], [
      ...(navigation === 'previous' ? [h.button([h.Type('button'), h.AriaLabel('Vorheriger Monat'), h.OnClick(toParentMessage(State.Message.ShiftedTanStackCalendarMonth({ delta: -1 }))), h.Class(className(styles.calendarNavigation))], [Icon.chevronLeft({ class: className(styles.smallIcon) }, h)])] : [h.span([h.Class(className(styles.calendarNavigationSpacer))], [])]),
      h.strong([h.Class(className(styles.calendarTitle))], [new Intl.DateTimeFormat('de', { month: 'long', year: 'numeric' }).format(monthStart)]),
      ...(navigation === 'next' ? [h.button([h.Type('button'), h.AriaLabel('Nächster Monat'), h.Disabled(model.calendarMonthOffset >= 0), h.OnClick(toParentMessage(State.Message.ShiftedTanStackCalendarMonth({ delta: 1 }))), h.Class(className(styles.calendarNavigation, model.calendarMonthOffset >= 0 && styles.calendarDayDisabled))], [Icon.chevronRight({ class: className(styles.smallIcon) }, h)])] : [h.span([h.Class(className(styles.calendarNavigationSpacer))], [])]),
    ]),
...
const filterPanel = <Data, Message>(column: TanStackDataTableColumn<Data, Message>, props: TanStackDataTableProps<Data, Message>, h: HtmlBuilder<Message>): Html => {
  ...
  const clear = props.toParentMessage(State.Message.ClearedTanStackFilterPopover({ columnId: column.id }))
  if (column.filter?.type === 'text') return h.div([h.Style({ positionAnchor: '--data-table-filter' }), h.Class(className(styles.filterPanel, styles.textFilterPanel))], [
    h.input([h.Type('text'), h.Value(props.model.filterDraft), h.Placeholder('Suchen…'), h.AriaLabel(`${column.header} durchsuchen`), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackFilterDraft({ value }))), h.OnKeyDownPreventDefault((key) => key === 'Enter' ? Option.some(props.toParentMessage(State.Message.AppliedTanStackTextFilter({ columnId: column.id }))) : Option.none()), h.Class(className(styles.filterInput))]),
    h.div([h.Class(className(styles.filterActions))], [button({ children: ['Filtern'], size: 'sm', layoutStyle: styles.filterApply, onClick: props.toParentMessage(State.Message.AppliedTanStackTextFilter({ columnId: column.id })) }, h), ...(filter === undefined ? [] : [button({ children: [Icon.x({ class: className(styles.smallIcon) }, h)], size: 'icon', variant: 'outline', onClick: clear }, h)])]),
  ])
  if (column.filter?.type === 'enum') {
    ...
    return h.div([h.Style({ positionAnchor: '--data-table-filter' }), h.Class(className(styles.filterPanel, styles.enumFilterPanel))], [
      ...
      ...(options.length <= 5 ? [] : [h.input([h.Type('search'), h.Value(props.model.enumSearch), h.Placeholder('Suchen…'), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackEnumSearch({ value }))), h.Class(className(styles.filterInput))])]),
  ...
  if (column.filter?.type === 'number') {
    ...
    const apply = props.toParentMessage(State.Message.AppliedTanStackColumnFilter({ filter: State.columnFilter(column.id, 'number', { value: props.model.filterDraft, secondaryValue: props.model.filterSecondaryDraft, operator: props.model.filterOperator }) }))
    return h.div([h.Style({ positionAnchor: '--data-table-filter' }), h.Class(className(styles.filterPanel, styles.numberFilterPanel))], [
      h.label([h.Class(className(styles.field))], [h.span([], ['Bedingung']), h.select([h.Value(props.model.filterOperator), h.OnChange((value) => props.toParentMessage(State.Message.ChangedTanStackFilterOperator({ operator: value === 'gte' || value === 'lte' || value === 'between' ? value : 'eq' }))), h.Class(className(styles.select))], [h.option([h.Value('eq')], ['Ist gleich']), h.option([h.Value('gte')], ['Größer oder gleich']), h.option([h.Value('lte')], ['Kleiner oder gleich']), h.option([h.Value('between')], ['Zwischen'])])]),
      h.div([h.Class(className(props.model.filterOperator === 'between' && styles.twoFields))], [h.label([h.Class(className(styles.field))], [h.span([], [props.model.filterOperator === 'between' ? 'Von' : 'Wert']), h.div([h.Class(className(styles.inputWithUnit))], [h.input([h.Type('number'), h.Step(String(column.filter.step ?? 1)), h.Value(props.model.filterDraft), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackFilterDraft({ value }))), h.Class(className(styles.filterInput))]), ...(column.filter.unit ? [h.span([], [column.filter.unit])] : [])])]), ...(props.model.filterOperator !== 'between' ? [] : [h.label([h.Class(className(styles.field))], [h.span([], ['Bis']), h.div([h.Class(className(styles.inputWithUnit))], [h.input([h.Type('number'), h.Step(String(column.filter.step ?? 1)), h.Value(props.model.filterSecondaryDraft), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackFilterSecondaryDraft({ value }))), h.Class(className(styles.filterInput))]), ...(column.filter.unit ? [h.span([], [column.filter.unit])] : [])])])])]),
  ...
  const now = new Date()
  const preset = (hours: number) => props.toParentMessage(State.Message.AppliedTanStackColumnFilter({ filter: State.columnFilter(column.id, 'datetime', { value: new Date(now.getTime() - hours * 3_600_000).toISOString(), secondaryValue: now.toISOString() }) }))
  const applyDate = props.toParentMessage(State.Message.AppliedTanStackColumnFilter({ filter: State.columnFilter(column.id, 'datetime', { value: props.model.filterDraft, secondaryValue: props.model.filterSecondaryDraft }) }))
  if (!props.model.dateCustomOpen) return h.div([h.Style({ positionAnchor: '--data-table-filter' }), h.Class(className(styles.filterPanel, styles.datePresetPanel))], [h.div([h.Class(className(styles.datePresetList))], [
    ...
    h.button([h.Type('button'), h.OnClick(props.toParentMessage(State.Message.ChangedTanStackDateCustomOpen({ isOpen: true }))), h.Class(className(styles.datePresetButton))], [Icon.calendarDays({ class: className(styles.smallIcon) }, h), 'Benutzerdefiniert…']),
  ])])
  return h.div([h.Style({ positionAnchor: '--data-table-filter' }), h.Class(className(styles.filterPanel, styles.dateFilterPanel))], [
    ...
    h.div([h.Class(className(styles.dateFields))], [h.label([h.Class(className(styles.field))], [h.span([], ['Von']), h.input([h.Type('datetime-local'), h.Value(localDateTimeValue(props.model.filterDraft)), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackFilterDraft({ value }))), h.Class(className(styles.filterInput))])]), h.label([h.Class(className(styles.field))], [h.span([], ['Bis']), h.input([h.Type('datetime-local'), h.Value(localDateTimeValue(props.model.filterSecondaryDraft)), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackFilterSecondaryDraft({ value }))), h.Class(className(styles.filterInput))])])]),
...
const filterControl = <Data, Message>(column: TanStackDataTableColumn<Data, Message>, props: TanStackDataTableProps<Data, Message>, h: HtmlBuilder<Message>): Html => {
  ...
  return h.div([h.Style(open ? { anchorName: '--data-table-filter' } : {}), h.Class(className(styles.filterControl))], [h.button([h.Type('button'), h.Title('Spalte filtern'), h.AriaLabel(`${column.header} filtern`), h.AriaExpanded(open), h.OnClick(props.toParentMessage(State.Message.ToggledTanStackFilterPopover({ columnId: column.id, draft: filter?.value ?? '', secondaryDraft: filter?.secondaryValue ?? '', operator: filter?.operator ?? 'eq' }))), h.Class(className(styles.filterTrigger, filter !== undefined && styles.filterTriggerActive))], [Icon.filter({ class: className(styles.filterIcon) }, h)]), ...(open ? [filterPanel(column, props, h)] : [])])
}

const layoutMenu = <Data, Message>(ordered: ReadonlyArray<TanStackDataTableColumn<Data, Message>>, props: TanStackDataTableProps<Data, Message>, h: HtmlBuilder<Message>): Html => h.div([h.Class(className(styles.layoutWrap))], [button({ children: [Icon.chevronsUpDown({ class: className(styles.smallIcon) }, h), 'Spalten anpassen'], size: 'sm', variant: props.model.hiddenColumnIds.length > 0 ? 'default' : 'outline', onClick: props.toParentMessage(State.Message.ToggledTanStackLayoutMenu()) }, h), ...(props.model.layoutMenuOpen ? [h.div([h.Class(className(styles.layoutPanel))], [h.div([h.Class(className(styles.layoutHeader))], [h.strong([], ['Spalten anpassen']), button({ children: ['Zurücksetzen'], size: 'sm', variant: 'ghost', onClick: props.toParentMessage(State.Message.ResetTanStackTableLayout({ columnOrder: ['select', ...props.columns.map((column) => column.id)], pinnedColumnIds: ['select', ...props.columns.filter((column) => column.sticky).map((column) => column.id)] })) }, h)]), h.div([h.Class(className(styles.layoutList))], ordered.map((column, index) => {
  ...
  return h.div([h.Class(className(styles.layoutItem, hidden && styles.layoutItemMuted))], [button({ children: [Icon.arrowUp({ class: className(styles.tinyIcon) }, h)], size: 'icon', variant: 'ghost', isDisabled: index === 0, onClick: props.toParentMessage(State.Message.MovedTanStackColumn({ columnId: column.id, direction: 'left' })) }, h), button({ children: [Icon.arrowDown({ class: className(styles.tinyIcon) }, h)], size: 'icon', variant: 'ghost', isDisabled: index === ordered.length - 1, onClick: props.toParentMessage(State.Message.MovedTanStackColumn({ columnId: column.id, direction: 'right' })) }, h), h.button([h.Type('button'), h.OnClick(props.toParentMessage(State.Message.ToggledTanStackColumn({ columnId: column.id, isVisible: hidden }))), h.Class(className(styles.visibilityButton))], [hidden ? Icon.eyeOff({ class: className(styles.smallIcon) }, h) : Icon.eye({ class: className(styles.smallIcon) }, h), h.span([h.Class(className(styles.truncate))], [column.header])]), button({ children: [Icon.pin({ class: className(styles.smallIcon) }, h)], size: 'icon', variant: pinned ? 'secondary' : 'ghost', isDisabled: hidden, onClick: props.toParentMessage(State.Message.ToggledTanStackColumnPin({ columnId: column.id })) }, h)])
})), ])] : [])])
...
export const tanStackDataTable = <Data, Message>(props: TanStackDataTableProps<Data, Message>, h: HtmlBuilder<Message>): Html => {
  ...
  const backdrop = props.model.openFilterColumnId !== '' || props.model.layoutMenuOpen || props.model.activeFiltersMenuOpen || props.model.pageSizeMenuOpen ? h.button([h.Type('button'), h.AriaLabel('Menü schließen'), h.OnClick(props.toParentMessage(State.Message.ClosedTanStackTableOverlays())), h.Class(className(styles.backdrop, props.model.openFilterColumnId !== '' && styles.filterBackdrop))], []) : h.empty
  const modifiedToolbar = modified && props.storageKey !== undefined ? h.div([h.Class(className(styles.modifiedToolbar))], [
    h.button([h.Type('button'), h.Title('Gesamte Ansicht zurücksetzen'), h.AriaLabel('Gesamte Ansicht zurücksetzen'), h.OnClick(props.toParentMessage(State.Message.ResetTanStackTableView())), h.Class(className(styles.toolbarButton))], [Icon.rotateCcw({ class: className(styles.smallIcon) }, h)]),
    h.button([h.Type('button'), h.Title('Spaltenbreiten zurücksetzen'), h.AriaLabel('Spaltenbreiten zurücksetzen'), h.OnClick(props.toParentMessage(State.Message.ResetTanStackColumnWidths())), h.Class(className(styles.toolbarButton))], [Icon.moveHorizontal({ class: className(styles.smallIcon) }, h)]),
    ...(props.model.columnFilters.length === 0 ? [] : [h.div([h.Class(className(styles.activeFilterWrap))], [
      h.button([h.Type('button'), h.Title('Aktive Filter'), h.AriaLabel('Aktive Filter'), h.OnClick(props.toParentMessage(State.Message.ToggledTanStackActiveFiltersMenu())), h.Class(className(styles.toolbarButton))], [Icon.filter({ class: className(styles.smallIcon) }, h), h.span([h.Class(className(styles.filterBadge))], [String(props.model.columnFilters.length)])]),
      ...(props.model.activeFiltersMenuOpen ? [h.div([h.Class(className(styles.activeFilterPanel))], [
        ...
        ...props.model.columnFilters.map((filter) => {
          ...
          return h.div([h.Class(className(styles.activeFilterItem))], [
            ...
            h.button([h.Type('button'), h.AriaLabel(`${column?.header ?? filter.columnId} Filter entfernen`), h.OnClick(props.toParentMessage(State.Message.ClearedTanStackColumnFilter({ columnId: filter.columnId }))), h.Class(className(styles.iconButton))], [Icon.x({ class: className(styles.tinyIcon) }, h)]),
          ])
        }),
      ])] : []),
    ])]),
  ]) : h.empty
  return h.div([h.Role('table'), h.AriaLabel(props.ariaLabel), h.OnPointerMove((screenX) => props.model.resizingColumnId === '' ? Option.none() : Option.some(props.toParentMessage(State.Message.DraggedTanStackColumnResize({ screenX })))), h.OnPointerUp(() => props.model.resizingColumnId === '' ? Option.none() : Option.some(props.toParentMessage(State.Message.EndedTanStackColumnResize()))), h.Class(className(styles.root, props.model.resizingColumnId !== '' && styles.resizing))], [backdrop,
    ...
    h.div([h.Class(className(styles.tableColumn))], [h.div([h.Class(className(styles.tableFrame, props.storageKey !== undefined && modified && styles.tableFrameModified))], [props.isLoading === true ? h.div([h.Role('status'), h.Class(className(styles.loading))], [Icon.loaderCircle({ class: className(styles.spinner) }, h), props.loadingText ?? 'Wird geladen…']) : h.div([
      ...
      h.OnMount({ name: `tanstack-table-scroll-${props.storageKey ?? props.ariaLabel}`, f: (element) => element instanceof HTMLElement ? Subscription.fromEvent<HTMLElement, 'scroll', Message>({ target: element, type: 'scroll', mapEvent: () => props.toParentMessage(State.Message.ClosedTanStackTableOverlays()), options: { passive: true } }) : Stream.empty }),
    ], [
      h.div([h.Role('row'), h.Style({ gridTemplateColumns: templateColumns }), h.Class(className(styles.grid, styles.header))], [
        ...(selection ? [h.div([h.Role('columnheader'), h.Class(className(styles.prefixCell, pinned.length > 0 && styles.stickyHeader, pinned.length > 0 && styles.stickyPrefixHeader, pinned.length > 0 && styles.stickyLeading)), h.Style(pinned.length > 0 ? { left: '16px' } : {})], [checkbox(allSelected, someSelected, 'Alle auswählen', props.toParentMessage(State.Message.ToggledTanStackRows({ rowIds: allIds, isSelected: !allSelected })), h)])] : []),
        ...
        ...visible.map((column, index) => {
          const isPinned = props.model.pinnedColumnIds.includes(column.id)
          const currentSort = props.model.sorting[0]
          const nextSort = currentSort?.id === column.id ? [{ id: column.id, desc: !currentSort.desc }] : [{ id: column.id, desc: false }]
          const label = [
            h.span([h.Class(className(styles.truncate))], [column.header]),
            ...(column.canSort === false ? [] : [Icon.icon(currentSort?.id === column.id ? currentSort.desc ? 'arrow-down' : 'arrow-up' : 'chevrons-up-down', { class: className(styles.sortIcon, currentSort?.id !== column.id && styles.sortIconIdle) }, h)]),
            ...(column.headerTooltip === undefined ? [] : [h.span([h.Title(column.headerTooltip), h.AriaLabel(`Hilfe zu ${column.header}`), h.Class(className(styles.info))], [Icon.info({ class: className(styles.smallIcon) }, h)])]),
          ]
          const header = column.canSort === false
            ? h.span([h.Class(className(styles.sortButton, styles.staticHeader))], label)
            : h.button([h.Type('button'), h.OnClick(props.toParentMessage(State.Message.ChangedTanStackSorting({ sorting: nextSort }))), h.Class(className(styles.sortButton))], label)
          return h.div([h.Role('columnheader'), h.Style(isPinned ? { left: `${stickyOffset(column)}px` } : {}), h.Class(className(styles.headerCell, index > 0 && styles.cellInset, isPinned && styles.stickyHeader))], [h.div([h.Class(className(styles.headerInner))], [header, filterControl(column, props, h)]), ...(props.storageKey === undefined || index === visible.length - 1 ? [] : [h.div([h.Role('separator'), h.AriaLabel('Spaltenbreite anpassen'), h.Title('Spaltenbreite anpassen'), h.OnPointerDown((_type, pointerButton, screenX) => pointerButton === 0 ? Option.some(props.toParentMessage(State.Message.StartedTanStackColumnResize({ columnId: column.id, screenX, width: widths[index] ?? 120 }))) : Option.none()), h.Class(className(styles.resizeHandle))], [h.span([h.Class(className(styles.resizeDots))], [])])])])
        }),
      ]),
      h.div([h.Role('rowgroup'), h.Class(className(styles.rows))], pageRows.length === 0 ? [h.div([h.Class(className(styles.empty))], [props.emptyText ?? 'Keine Daten gefunden'])] : pageRows.flatMap((row) => { const selected = props.model.selectedRowIds.includes(row.id); const expanded = props.model.expandedRowIds.includes(row.id); const dataCells = visible.map((column, index) => { const isPinned = props.model.pinnedColumnIds.includes(column.id); return h.div([h.Role('cell'), ...(expandable || props.onRowClick !== undefined ? [h.OnClick(props.onRowClick?.(row.original) ?? props.toParentMessage(State.Message.ToggledTanStackExpanded({ rowId: row.id })))] : []), h.Style(isPinned ? { left: `${stickyOffset(column)}px` } : {}), h.Class(className(styles.bodyCell, index > 0 && styles.cellInset, isPinned && styles.stickyBody, selected && styles.stickySelected, expanded && styles.stickyExpanded))], [column.cell?.(row.original, h) ?? String(column.value(row.original) ?? '–')]) }); return [h.div([h.Role('row'), h.Key(row.id), h.Style({ gridTemplateColumns: templateColumns }), h.Class(className(styles.grid, styles.row, (expandable || props.onRowClick !== undefined) && styles.rowInteractive, selected && styles.rowSelected, expanded && styles.rowExpanded))], [...(selection ? [h.div([h.Role('cell'), h.Class(className(styles.prefixCell, pinned.length > 0 && styles.stickyBody, pinned.length > 0 && styles.stickyPrefixBody, pinned.length > 0 && styles.stickyLeading, selected && styles.stickySelected, expanded && styles.stickyExpanded)), h.Style(pinned.length > 0 ? { left: '16px' } : {})], [checkbox(selected, false, `Zeile ${row.id} auswählen`, props.toParentMessage(State.Message.ToggledTanStackRow({ rowId: row.id, isSelected: !selected })), h)])] : []), ...(expandable ? [h.div([h.Role('cell'), h.OnClick(props.toParentMessage(State.Message.ToggledTanStackExpanded({ rowId: row.id }))), h.Class(className(styles.prefixCell, pinned.length > 0 && styles.stickyBody, pinned.length > 0 && styles.stickyPrefixBody, pinned.length > 0 && !selection && styles.stickyLeading, selected && styles.stickySelected, expanded && styles.stickyExpanded)), h.Style(pinned.length > 0 ? { left: `${16 + (selection ? 40 : 0)}px` } : {})], [Icon.chevronDown({ class: className(styles.expandIcon, expanded && styles.expandIconOpen) }, h)])] : []), ...dataCells]), ...(expandable && expanded ? [h.div([h.Class(className(styles.expandedContent))], [props.expandedContent?.(row.original, h) ?? 'Erweiterte Zeile'])] : [])] })),
    ])]),
    ...(props.storageKey === undefined && props.footerContent === undefined && props.pagination === undefined ? [] : [h.div([h.Class(className(styles.footer))], [h.div([h.Class(className(styles.footerLeft))], [...(props.footerContent === undefined ? [] : [props.footerContent]), h.span([h.Class(className(styles.footerText))], [totalCount > props.model.pageSize ? `Seite ${props.model.pageIndex + 1} von ${pageCount}` : `${totalCount} Einträge`]), ...(props.pageSizeOptions === undefined ? [] : [h.div([h.Class(className(styles.pageSizeWrap))], [h.span([h.Class(className(styles.footerText))], ['Zeilen:']), h.input([h.Type('text'), h.InputMode('numeric'), h.Value(props.model.pageSizeMenuOpen ? props.model.pageSizeDraft : String(props.model.pageSize)), h.OnFocus(props.toParentMessage(State.Message.ToggledTanStackPageSizeMenu())), h.OnInput((value) => props.toParentMessage(State.Message.ChangedTanStackPageSizeDraft({ value }))), h.OnKeyDownPreventDefault((key) => key === 'Enter' && Number(props.model.pageSizeDraft) > 0 ? Option.some(props.toParentMessage(State.Message.CommittedTanStackPageSize({ pageSize: Number(props.model.pageSizeDraft) }))) : Option.none()), h.Class(className(styles.pageSizeInput))]), ...(props.model.pageSizeMenuOpen ? [h.div([h.Class(className(styles.pageSizePanel))], props.pageSizeOptions.map((size) => h.button([h.Type('button'), h.OnClick(props.toParentMessage(State.Message.CommittedTanStackPageSize({ pageSize: size }))), h.Class(className(styles.pageSizeOption, size === props.model.pageSize && styles.pageSizeOptionActive))], [String(size)])))] : [])])])]), h.div([h.Class(className(styles.footerRight))], [layoutMenu(ordered, props, h), ...(totalCount <= props.model.pageSize ? [] : [button({ children: ['Zurück'], size: 'sm', variant: 'outline', isDisabled: props.model.pageIndex === 0, onClick: props.toParentMessage(State.Message.ChangedTanStackPage({ pageIndex: Math.max(0, props.model.pageIndex - 1) })) }, h), button({ children: ['Weiter'], size: 'sm', variant: 'outline', isDisabled: props.pagination?.hasMore === false || (props.model.pageIndex + 1) * props.model.pageSize >= totalCount, onClick: props.toParentMessage(State.Message.ChangedTanStackPage({ pageIndex: props.model.pageIndex + 1 })) }, h)])])])]),
  ])])
}
