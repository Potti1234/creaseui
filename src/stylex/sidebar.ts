...
export type SidebarProviderProps = Slot & Readonly<{ state?: SidebarState; belowHeader?: boolean; width?: string; mobileWidth?: string; iconWidth?: string }>
export const sidebarProvider = <Msg>(props: SidebarProviderProps, h: HtmlBuilder<Msg>): Html => h.div([h.DataAttribute('renderer', 'stylex'), h.DataAttribute('slot', 'sidebar-wrapper'), h.DataAttribute('state', props.state ?? 'expanded'), h.Style({ '--sidebar-width': props.width ?? '16rem', '--sidebar-width-mobile': props.mobileWidth ?? '18rem', '--sidebar-width-icon': props.iconWidth ?? '3rem' }), h.Class(className(styles.wrapper, props.belowHeader === true && styles.wrapperBelowHeader, props.layoutStyle))], [...props.children])

export type SidebarProps<Msg> = Slot & Readonly<{ state?: SidebarState; side?: SidebarSide; variant?: SidebarVariant; collapsible?: SidebarCollapsible; surface?: 'default' | 'transparent'; presentation?: 'application' | 'contained'; belowHeader?: boolean; isMobileOpen?: boolean; onMobileDismiss?: Msg }>
