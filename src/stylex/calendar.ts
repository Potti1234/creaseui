...
const styles = stylex.create({
  ...
  nav: { gap: '0.25rem', alignItems: 'center', display: 'flex', justifyContent: 'space-between', pointerEvents: 'none', position: 'absolute', left: 0, right: 0, top: 0, width: '100%', },
  navButton: { padding: 0, borderRadius: tokens.controlRadius, alignItems: 'center', backgroundColor: { default: tokens.transparent, ':hover': tokens.accent }, display: 'flex', justifyContent: 'center', pointerEvents: 'auto', height: '2rem', width: '2rem', },
...
export type CalendarViewOptions = Readonly<{ direction?: 'ltr' | 'rtl'; layoutStyle?: ComponentLayoutStyle; range?: CalendarBehavior.CalendarRange; size?: 'default' | 'comfortable'; weekNumbers?: boolean }>
...
const daysView = <Msg>(attributes: CalendarPrimitive.DaysModeAttributes, options: CalendarViewOptions, h: HtmlBuilder<Msg>): Html => {
  ...
  const weeks = attributes.weeks.map((week) => h.div(
    ...
    [
      ...(options.weekNumbers === true
        ? [h.div([h.Class(className(styles.weekday, options.size === 'comfortable' && styles.weekdayComfortable)), h.DataAttribute('slot', 'calendar-week-number')], [week.cells[0] === undefined ? '' : String(isoWeekNumber(week.cells[0].date))])]
        : []),
  ...
  return h.div([...attributes.root, ...(options.direction === undefined ? [] : [h.Dir(options.direction)]), h.DataAttribute('slot', 'calendar'), h.Class(className(styles.root, options.layoutStyle))], [h.div([h.Class(className(styles.month))], [
    ...
    h.div([...attributes.grid, h.Class(className(styles.grid))], [h.div([...attributes.headerRow, h.Class(className(styles.headerRow))], [
      ...(options.weekNumbers === true
        ? [h.div([h.Class(className(styles.weekday, options.size === 'comfortable' && styles.weekdayComfortable))], [])]
        : []),
      ...weekdays,
    ]), ...weeks]),
  ])])
...
export type CalendarProps<Msg> = Readonly<{ model: Model; maybeSelectedDate: Option.Option<FoldkitCalendar.CalendarDate>; toParentMessage: (message: Message) => Msg; direction?: 'ltr' | 'rtl'; range?: CalendarBehavior.CalendarRange; layoutStyle?: ComponentLayoutStyle; size?: 'default' | 'comfortable'; weekNumbers?: boolean; previousMonthLabel?: string; nextMonthLabel?: string; previousYearsPageLabel?: string; nextYearsPageLabel?: string; daysHeadingButtonLabel?: string; monthsHeadingButtonLabel?: string }>
export const calendar = <Msg>(props: CalendarProps<Msg>, h: HtmlBuilder<Msg>): Html => h.submodel({ slotId: props.model.id, model: props.model, view: CalendarPrimitive.view, viewInputs: { maybeSelectedDate: props.maybeSelectedDate, toView: (attributes) => calendarView(attributes, { ...(props.direction === undefined ? {} : { direction: props.direction }), ...(props.range === undefined ? {} : { range: props.range }), ...(props.layoutStyle === undefined ? {} : { layoutStyle: props.layoutStyle }), ...(props.size === undefined ? {} : { size: props.size }), ...(props.weekNumbers === undefined ? {} : { weekNumbers: props.weekNumbers }) }, h), ...(props.previousMonthLabel === undefined ? {} : { previousMonthLabel: props.previousMonthLabel }), ...(props.nextMonthLabel === undefined ? {} : { nextMonthLabel: props.nextMonthLabel }), ...(props.previousYearsPageLabel === undefined ? {} : { previousYearsPageLabel: props.previousYearsPageLabel }), ...(props.nextYearsPageLabel === undefined ? {} : { nextYearsPageLabel: props.nextYearsPageLabel }), ...(props.daysHeadingButtonLabel === undefined ? {} : { daysHeadingButtonLabel: props.daysHeadingButtonLabel }), ...(props.monthsHeadingButtonLabel === undefined ? {} : { monthsHeadingButtonLabel: props.monthsHeadingButtonLabel }) }, toParentMessage: message => props.toParentMessage(props.direction === 'rtl' ? CalendarBehavior.mirrorNavigationKeyForRtl(message) : message) })
