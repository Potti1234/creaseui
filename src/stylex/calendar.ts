...
export type CalendarViewOptions = Readonly<{ layoutStyle?: ComponentLayoutStyle; size?: 'default' | 'comfortable' }>
const navigationButton = <Msg>(attributes: ReadonlyArray<ChildAttribute>, direction: 'previous' | 'next', options: CalendarViewOptions, h: HtmlBuilder<Msg>): Html => h.button([...attributes, h.Class(className(styles.navButton, options.size === 'comfortable' && styles.navButtonComfortable))], [direction === 'previous' ? Icon.chevronLeft({ class: className(styles.icon) }, h) : Icon.chevronRight({ class: className(styles.icon) }, h)])
...
const daysView = <Msg>(attributes: CalendarPrimitive.DaysModeAttributes, options: CalendarViewOptions, h: HtmlBuilder<Msg>): Html => {
  ...
  const weeks = attributes.weeks.map((week) => h.div(
    ...
    week.cells.map((cell) => h.div([...cell.cellAttributes, h.Class(className(styles.dayCell, options.size === 'comfortable' && styles.dayCellComfortable))], [h.button([...cell.buttonAttributes, h.Class(className(styles.dayButton, options.size === 'comfortable' && styles.dayButtonComfortable))], [cell.label])])),
  ))
  return h.div([...attributes.root, h.DataAttribute('slot', 'calendar'), h.Class(className(styles.root, options.layoutStyle))], [h.div([h.Class(className(styles.month))], [
...
export type CalendarProps<Msg> = Readonly<{ model: Model; maybeSelectedDate: Option.Option<FoldkitCalendar.CalendarDate>; toParentMessage: (message: Message) => Msg; layoutStyle?: ComponentLayoutStyle; size?: 'default' | 'comfortable'; previousMonthLabel?: string; nextMonthLabel?: string; previousYearsPageLabel?: string; nextYearsPageLabel?: string; daysHeadingButtonLabel?: string; monthsHeadingButtonLabel?: string }>
export const calendar = <Msg>(props: CalendarProps<Msg>, h: HtmlBuilder<Msg>): Html => h.submodel({ slotId: props.model.id, model: props.model, view: CalendarPrimitive.view, viewInputs: { maybeSelectedDate: props.maybeSelectedDate, toView: (attributes) => calendarView(attributes, { ...(props.layoutStyle === undefined ? {} : { layoutStyle: props.layoutStyle }), ...(props.size === undefined ? {} : { size: props.size }) }, h), ...(props.previousMonthLabel === undefined ? {} : { previousMonthLabel: props.previousMonthLabel }), ...(props.nextMonthLabel === undefined ? {} : { nextMonthLabel: props.nextMonthLabel }), ...(props.previousYearsPageLabel === undefined ? {} : { previousYearsPageLabel: props.previousYearsPageLabel }), ...(props.nextYearsPageLabel === undefined ? {} : { nextYearsPageLabel: props.nextYearsPageLabel }), ...(props.daysHeadingButtonLabel === undefined ? {} : { daysHeadingButtonLabel: props.daysHeadingButtonLabel }), ...(props.monthsHeadingButtonLabel === undefined ? {} : { monthsHeadingButtonLabel: props.monthsHeadingButtonLabel }) }, toParentMessage: props.toParentMessage })
