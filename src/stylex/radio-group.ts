...
export const radioGroup=<Msg>(p:RadioGroupProps<Msg>,h:HtmlBuilder<Msg>):Html=>renderRadioGroup(p,{group:[h.Class(className(styles.group,p.columns===2&&styles.groupColumns2,p.layoutStyle))],row:[h.Class(className(styles.row))],item:state=>[h.Class(className(styles.item,(state.isDisabled||state.isReadOnly)&&styles.disabled,p.options[state.index]?.isInvalid===true&&styles.invalid))],indicator:[h.Class(className(styles.indicator))],text:[h.Class(className(styles.text))],label:[h.Class(className(styles.label))],description:[h.Class(className(styles.description))]},(isSelected,indicatorH)=>isSelected?Icon.circleIcon({class:className(styles.dot)},indicatorH):indicatorH.empty,h)
