...
export type MultiSliderProps<Msg> = Readonly<{ values: readonly number[]; min: number; max: number; step?: number; onInput: (values: readonly number[]) => Msg; orientation?: 'horizontal' | 'vertical'; direction?: 'ltr' | 'rtl'; ariaLabels?: readonly string[]; isDisabled?: boolean; isReadOnly?: boolean; name?: string; layoutStyle?: ComponentLayoutStyle }>

export const multiSlider = <Msg>(props: MultiSliderProps<Msg>, h: HtmlBuilder<Msg>): Html => {
  const range = normalizeRange(props.min, props.max, props.step); const values = normalizeMultiValues(props.values, range); const span = Math.max(range.max - range.min, 1); const orientation = props.orientation ?? 'horizontal'; const rtl = props.direction === 'rtl' && orientation === 'horizontal'; const percent = (value: number): number => ((value - range.min) / span) * 100
  const segments: ReadonlyArray<readonly [number, number]> = values.length === 0 ? [] : values.length === 1 ? [[range.min, values[0] ?? range.min]] : values.slice(0, -1).map((value, index) => [value, values[index + 1] ?? value])
  const input = (index: number, value: number): Html => h.input([h.Type('range'), h.Min(String(range.min)), h.Max(String(range.max)), h.Step(String(range.step)), h.Value(String(value)), h.AriaLabel(props.ariaLabels?.[index] ?? `Value ${String(index + 1)}`), ...(props.isReadOnly === true ? [h.AriaReadonly(true)] : []), h.Disabled(props.isDisabled ?? false), ...(props.name === undefined ? [] : [h.Name(`${props.name}[${String(index)}]`)]), h.OnInput((next) => props.onInput(props.isReadOnly === true ? values : updateMultiValue(values, index, Number(next), range))), h.Class(className(styles.input, orientation === 'horizontal' ? styles.inputHorizontal : styles.inputVertical))])
  return h.div([h.DataAttribute('slot', 'slider'), h.DataAttribute('orientation', orientation), ...(props.direction === undefined ? [] : [h.Dir(props.direction)]), ...(props.isDisabled === true ? [h.DataAttribute('disabled', '')] : []), ...(props.isReadOnly === true ? [h.DataAttribute('readonly', '')] : []), h.Class(className(styles.control, orientation === 'horizontal' ? styles.horizontal : styles.vertical, props.isDisabled === true && styles.disabled, props.layoutStyle))], [h.div([h.DataAttribute('slot', 'slider-track'), h.Class(className(styles.track, orientation === 'horizontal' ? styles.trackHorizontal : styles.trackVertical))], segments.map(([lower, upper]) => { const start = percent(lower); const end = percent(upper); return h.div([h.DataAttribute('slot', 'slider-range'), h.Class(className(styles.range)), h.Style(orientation === 'horizontal' ? (rtl ? { right: `${start}%`, left: `${100 - end}%`, insetBlock: '0' } : { left: `${start}%`, right: `${100 - end}%`, insetBlock: '0' }) : { bottom: `${start}%`, top: `${100 - end}%`, insetInline: '0' })], []) })), ...values.map((value, index) => input(index, value))])
}
