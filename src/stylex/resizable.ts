...
export const Message = defineMessageUnion({
  ...
  DraggedResize: { position: S.Number, extent: S.Number },
  EndedResize: {},
...
export type ResizableProps<Msg> = Readonly<{ model: Model; toParentMessage: (message: Message) => Msg; first: Html | string; second: Html | string; direction?: 'horizontal' | 'vertical'; rtl?: boolean; withHandle?: boolean; minSize?: number; maxSize?: number; extent?: number; keyboardStep?: number; disabled?: boolean; ariaLabel?: string; layoutStyle?: ComponentLayoutStyle }>
export const resizable = <Msg>(props: ResizableProps<Msg>, h: HtmlBuilder<Msg>): Html => {
  const vertical = props.direction === 'vertical'; const rtl = props.rtl === true && !vertical; const dragging = Option.isSome(props.model.drag); const minSize = clampTo(props.minSize ?? 10, 0, 100); const maxSize = clampTo(props.maxSize ?? 90, minSize, 100); const size = clampTo(props.model.firstSize, minSize, maxSize); const extent = Math.max(1, props.extent ?? (vertical ? window.innerHeight : window.innerWidth))
  const handleAttributes = [h.Role('separator'), h.AriaLabel(props.ariaLabel ?? 'Resize panels'), h.AriaOrientation(vertical ? 'horizontal' : 'vertical'), h.AriaValuemin(minSize), h.AriaValuemax(maxSize), h.AriaValuenow(Math.round(size)), h.AriaDisabled(props.disabled === true), h.Tabindex(props.disabled === true ? -1 : 0), h.DataAttribute('slot', 'resizable-handle'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), h.OnPointerDown((_type, button, screenX, screenY) => button === 0 && props.disabled !== true ? Option.some(props.toParentMessage(Message.StartedResize({ position: vertical ? screenY : rtl ? -screenX : screenX }))) : Option.none()), h.OnKeyDownPreventDefault((key, modifiers) => { if (props.disabled === true || modifiers.ctrlKey || modifiers.altKey || modifiers.metaKey) return Option.none(); const step = props.keyboardStep ?? 2; if (key === (vertical ? 'ArrowUp' : rtl ? 'ArrowRight' : 'ArrowLeft')) return Option.some(props.toParentMessage(Message.NudgedResize({ delta: -step }))); if (key === (vertical ? 'ArrowDown' : rtl ? 'ArrowLeft' : 'ArrowRight')) return Option.some(props.toParentMessage(Message.NudgedResize({ delta: step }))); if (key === 'Home') return Option.some(props.toParentMessage(Message.NudgedResize({ delta: minSize - size }))); if (key === 'End') return Option.some(props.toParentMessage(Message.NudgedResize({ delta: maxSize - size }))); return Option.none() })]
  return h.div([h.DataAttribute('slot', 'resizable-panel-group'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), ...(rtl ? [h.Dir('rtl')] : []), h.OnPointerMove((screenX, screenY) => dragging && props.disabled !== true ? Option.some(props.toParentMessage(Message.DraggedResize({ position: vertical ? screenY : rtl ? -screenX : screenX, extent }))) : Option.none()), h.OnPointerUp(() => dragging ? Option.some(props.toParentMessage(Message.EndedResize())) : Option.none()), h.Class(className(styles.group, vertical && styles.vertical, dragging && styles.dragging, props.layoutStyle))], [panel({ size, children: props.first }, h), handleView({ vertical, disabled: props.disabled === true, attributes: handleAttributes, withHandle: props.withHandle === true }, h), panel({ size: 100 - size, children: props.second }, h)])
}
...
export const GroupMessage = defineMessageUnion({
  StartedGroupResize: { handle: S.Number, position: S.Number },
  DraggedGroupResize: { position: S.Number, extent: S.Number, minSize: S.Number },
  EndedGroupResize: {},
  NudgedGroupResize: { handle: S.Number, delta: S.Number, minSize: S.Number },
});
...
export type ResizableGroupProps<Msg> = Readonly<{ model: GroupModel; toParentMessage: (message: GroupMessage) => Msg; panels: ReadonlyArray<Html | string>; direction?: 'horizontal' | 'vertical'; rtl?: boolean; minSize?: number; extent: number; withHandles?: boolean; disabled?: boolean; layoutStyle?: ComponentLayoutStyle }>
export const resizableGroup = <Msg>(props: ResizableGroupProps<Msg>, h: HtmlBuilder<Msg>): Html => {
  const vertical = props.direction === 'vertical'; const rtl = props.rtl === true && !vertical; const dragging = Option.isSome(props.model.drag); const minSize = Math.max(0, props.minSize ?? 5); const children: Array<Html> = []
  props.panels.forEach((child, index) => { children.push(panel({ size: props.model.sizes[index] ?? 0, children: child }, h)); if (index >= props.panels.length - 1) return; const value = props.model.sizes.slice(0, index + 1).reduce((sum, size) => sum + size, 0); const attributes = [h.Role('separator'), h.AriaOrientation(vertical ? 'horizontal' : 'vertical'), h.AriaValuemin(minSize), h.AriaValuemax(100 - minSize), h.AriaValuenow(Math.round(value)), h.AriaDisabled(props.disabled === true), h.Tabindex(props.disabled === true ? -1 : 0), h.DataAttribute('slot', 'resizable-handle'), h.OnPointerDown((_type, button, x, y) => button === 0 && props.disabled !== true ? Option.some(props.toParentMessage(GroupMessage.StartedGroupResize({ handle: index, position: vertical ? y : rtl ? -x : x }))) : Option.none()), h.OnKeyDownPreventDefault((key, modifiers) => props.disabled === true || modifiers.ctrlKey || modifiers.altKey || modifiers.metaKey ? Option.none() : key === (vertical ? 'ArrowUp' : rtl ? 'ArrowRight' : 'ArrowLeft') ? Option.some(props.toParentMessage(GroupMessage.NudgedGroupResize({ handle: index, delta: -2, minSize }))) : key === (vertical ? 'ArrowDown' : rtl ? 'ArrowLeft' : 'ArrowRight') ? Option.some(props.toParentMessage(GroupMessage.NudgedGroupResize({ handle: index, delta: 2, minSize }))) : Option.none())]; children.push(handleView({ vertical, disabled: props.disabled === true, attributes, withHandle: props.withHandles === true }, h)) })
  return h.div([h.DataAttribute('slot', 'resizable-panel-group'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), ...(rtl ? [h.Dir('rtl')] : []), h.OnPointerMove((x, y) => dragging && props.disabled !== true ? Option.some(props.toParentMessage(GroupMessage.DraggedGroupResize({ position: vertical ? y : rtl ? -x : x, extent: props.extent, minSize }))) : Option.none()), h.OnPointerUp(() => dragging ? Option.some(props.toParentMessage(GroupMessage.EndedGroupResize())) : Option.none()), h.Class(className(styles.group, vertical && styles.vertical, dragging && styles.dragging, props.layoutStyle))], children)
}
