...
export const resizable = <Msg>(props: ResizableProps<Msg>, h: HtmlBuilder<Msg>): Html => {
  ...
  const handleAttributes = [h.Role('separator'), h.AriaLabel(props.ariaLabel ?? 'Resize panels'), h.AriaOrientation(vertical ? 'horizontal' : 'vertical'), h.AriaValuemin(minSize), h.AriaValuemax(maxSize), h.AriaValuenow(Math.round(size)), h.AriaDisabled(props.disabled === true), h.Tabindex(props.disabled === true ? -1 : 0), h.DataAttribute('slot', 'resizable-handle'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), h.OnPointerDown((_type, button, screenX, screenY) => button === 0 && props.disabled !== true ? Option.some(props.toParentMessage(Message.StartedResize({ position: vertical ? screenY : screenX }))) : Option.none()), h.OnKeyDownPreventDefault((key, modifiers) => { if (props.disabled === true || modifiers.ctrlKey || modifiers.altKey || modifiers.metaKey) return Option.none(); const step = props.keyboardStep ?? 2; if (key === (vertical ? 'ArrowUp' : 'ArrowLeft')) return Option.some(props.toParentMessage(Message.NudgedResize({ delta: -step }))); if (key === (vertical ? 'ArrowDown' : 'ArrowRight')) return Option.some(props.toParentMessage(Message.NudgedResize({ delta: step }))); if (key === 'Home') return Option.some(props.toParentMessage(Message.NudgedResize({ delta: minSize - size }))); if (key === 'End') return Option.some(props.toParentMessage(Message.NudgedResize({ delta: maxSize - size }))); return Option.none() })]
  return h.div([h.DataAttribute('slot', 'resizable-panel-group'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), h.OnPointerMove((screenX, screenY) => dragging && props.disabled !== true ? Option.some(props.toParentMessage(Message.DraggedResize({ position: vertical ? screenY : screenX, extent }))) : Option.none()), h.OnPointerUp(() => dragging ? Option.some(props.toParentMessage(Message.EndedResize())) : Option.none()), h.Class(className(styles.group, vertical && styles.vertical, dragging && styles.dragging, props.layoutStyle))], [panel({ size, children: props.first }, h), handleView({ vertical, disabled: props.disabled === true, attributes: handleAttributes, withHandle: props.withHandle === true }, h), panel({ size: 100 - size, children: props.second }, h)])
}
