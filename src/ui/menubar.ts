...
export const menubar = <Item extends string, Msg>(
  ...
  return h.div(
    ...
    props.menus.map((menu, index) =>
      h.div(
        [h.Role('none'), h.DataAttribute('slot', 'menubar-menu'), h.OnKeyDownPreventDefault(key => {
          if (props.onMove === undefined || props.menus.length === 0) return Option.none()
          if (key !== 'ArrowLeft' && key !== 'ArrowRight') return Option.none()
          const delta = key === 'ArrowRight' ? 1 : -1
          return Option.some(props.onMove((index + delta + props.menus.length) % props.menus.length))
        })],
        [
          DropdownMenu.dropdownMenu<Item, Msg>({
            ...
            trigger: menu.label,
            triggerClass: cn('rounded-sm px-2 py-1 text-sm font-medium outline-none hover:bg-accent focus-visible:bg-accent data-[state=open]:bg-accent', props.activeIndex === index ? 'bg-accent' : undefined),
            items: menu.items,
