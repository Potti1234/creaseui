...
export const resizable = <Msg>(props: ResizableProps<Msg>): Html => {
  ...
  const extent = Math.max(1, props.extent ?? (vertical ? window.innerHeight : window.innerWidth))
  return h.div(
    [
      h.DataAttribute('slot', 'resizable-panel-group'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'),
      h.OnPointerMove((screenX, screenY) => dragging && props.disabled !== true ? Option.some(props.toParentMessage(DraggedResize({ position: vertical ? screenY : screenX, extent }))) : Option.none()),
    ...
    [
      panel({ size, children: props.first }),
      h.div(
        [h.Role('separator'), h.AriaLabel(props.ariaLabel ?? 'Resize panels'), h.AriaOrientation(vertical ? 'horizontal' : 'vertical'), h.AriaValuemin(minSize), h.AriaValuemax(maxSize), h.AriaValuenow(Math.round(size)), h.AriaDisabled(props.disabled === true), h.Tabindex(props.disabled === true ? -1 : 0), h.DataAttribute('slot', 'resizable-handle'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'), h.OnPointerDown((_type, button, screenX, screenY) => button === 0 && props.disabled !== true ? Option.some(props.toParentMessage(StartedResize({ position: vertical ? screenY : screenX }))) : Option.none()), h.OnKeyDownPreventDefault(key => {
          if (props.disabled === true) return Option.none()
          const step = props.keyboardStep ?? 2
          if (key === (vertical ? 'ArrowUp' : 'ArrowLeft')) return Option.some(props.toParentMessage(NudgedResize({ delta: -step })))
          if (key === (vertical ? 'ArrowDown' : 'ArrowRight')) return Option.some(props.toParentMessage(NudgedResize({ delta: step })))
          if (key === 'Home') return Option.some(props.toParentMessage(NudgedResize({ delta: minSize - size })))
          if (key === 'End') return Option.some(props.toParentMessage(NudgedResize({ delta: maxSize - size })))
          return Option.none()
        }), h.Class(cn('relative flex touch-none w-px items-center justify-center bg-border outline-none focus-visible:ring-1 focus-visible:ring-ring aria-disabled:cursor-not-allowed aria-disabled:opacity-50', vertical ? 'h-px w-full cursor-row-resize' : 'h-full w-px cursor-col-resize'))],
      ...
      panel({ size: 100 - size, children: props.second }),
    ],
  )
}
...
const normalizedSizes = (count: number, sizes?: ReadonlyArray<number>): Array<number> => {
  if (count <= 0) return []
  const source = sizes?.length === count ? sizes.map(value => Math.max(0, value)) : Array.from({ length: count }, () => 1)
...
const resizePair = (sizes: ReadonlyArray<number>, handle: number, delta: number, minSize: number): Array<number> => {
  const next = [...sizes]
...
export const updateGroup = (model: GroupModel, message: GroupMessage): GroupModel => {
  switch (message._tag) {
    case 'StartedGroupResize': {
      const before = model.sizes[message.handle]
      const after = model.sizes[message.handle + 1]
      return before === undefined || after === undefined ? model : { ...model, drag: Option.some({ handle: message.handle, start: message.position, before, after }) }
    }
    case 'DraggedGroupResize': return Option.match(model.drag, {
      onNone: () => model,
      onSome: drag => {
        const base = [...model.sizes]
        base[drag.handle] = drag.before
        base[drag.handle + 1] = drag.after
        return { ...model, sizes: resizePair(base, drag.handle, (message.position - drag.start) / Math.max(1, message.extent) * 100, message.minSize) }
      },
    })
    case 'EndedGroupResize': return { ...model, drag: Option.none() }
    case 'NudgedGroupResize': return { ...model, sizes: resizePair(model.sizes, message.handle, message.delta, message.minSize) }
  }
}
...
export const resizableGroup = <Msg>(props: ResizableGroupProps<Msg>): Html => {
  const h = html<Msg>()
  const vertical = props.direction === 'vertical'
  const dragging = Option.isSome(props.model.drag)
  ...
  props.panels.forEach((child, index) => {
    children.push(panel({ size: props.model.sizes[index] ?? 0, children: child }))
    if (index >= props.panels.length - 1) return
    const value = props.model.sizes.slice(0, index + 1).reduce((sum, size) => sum + size, 0)
    children.push(h.div([
      h.Role('separator'), h.AriaOrientation(vertical ? 'horizontal' : 'vertical'), h.AriaValuemin(minSize), h.AriaValuemax(100 - minSize), h.AriaValuenow(Math.round(value)), h.AriaDisabled(props.disabled === true), h.Tabindex(props.disabled === true ? -1 : 0), h.DataAttribute('slot', 'resizable-handle'),
      h.OnPointerDown((_type, button, x, y) => button === 0 && props.disabled !== true ? Option.some(props.toParentMessage(StartedGroupResize({ handle: index, position: vertical ? y : x }))) : Option.none()),
      h.OnKeyDownPreventDefault(key => props.disabled === true ? Option.none() : key === (vertical ? 'ArrowUp' : 'ArrowLeft') ? Option.some(props.toParentMessage(NudgedGroupResize({ handle: index, delta: -2, minSize }))) : key === (vertical ? 'ArrowDown' : 'ArrowRight') ? Option.some(props.toParentMessage(NudgedGroupResize({ handle: index, delta: 2, minSize }))) : Option.none()),
      h.Class(cn('relative flex touch-none items-center justify-center bg-border outline-none focus-visible:ring-1 focus-visible:ring-ring', vertical ? 'h-px w-full cursor-row-resize' : 'h-full w-px cursor-col-resize')),
    ], props.withHandles === true ? [Icon.gripVertical<Msg>({ class: cn('size-3', vertical ? 'rotate-90' : undefined) })] : []))
  })
  return h.div([
    h.DataAttribute('slot', 'resizable-panel-group'), h.DataAttribute('direction', vertical ? 'vertical' : 'horizontal'),
    h.OnPointerMove((x, y) => dragging && props.disabled !== true ? Option.some(props.toParentMessage(DraggedGroupResize({ position: vertical ? y : x, extent: props.extent, minSize }))) : Option.none()),
    h.OnPointerUp(() => dragging ? Option.some(props.toParentMessage(EndedGroupResize())) : Option.none()),
    h.Class(cn('flex size-full overflow-hidden rounded-lg border', vertical ? 'flex-col' : undefined, dragging ? 'select-none' : undefined, props.class)),
  ], children)
}
