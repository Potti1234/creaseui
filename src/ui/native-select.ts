...
export const nativeSelectOptGroup = <Msg>(
  ...
  return h.optgroup(
    ...
    props.options.map(option => nativeSelectOption<Msg>(option)),
  )
}
...
export const nativeSelect = <Msg>(props: NativeSelectProps<Msg>): Html => {
  const h = html<Msg>()

  return SelectPrimitive.view<Msg>({
    id: props.id,
    value: props.value,
          ...
          h.select(
            ...
            [
              ...props.options.map(option =>
                nativeSelectOption<Msg>(option),
              ),
              ...(props.groups ?? []).map(group =>
                nativeSelectOptGroup<Msg>(group),
              ),
            ],
          ),
          h.span(
            ...
            [Icon.chevronDown<Msg>({ class: 'size-4' })],
          ),
        ],
      )
